    ...
    <Protected>
      ...
      >
        <div className="h-[calc(100vh-120px)] min-h-[680px] w-full">
          <section className="relative flex h-full min-h-0 flex-col rounded-2xl border border-slate-200 bg-white shadow-sm">
            {isHistoryOpen ? (
              <div className="absolute right-4 top-14 z-20 w-80 rounded-xl border border-slate-200 bg-white p-3 shadow-xl">
                <p className="px-2 pb-2 text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Past chats</p>
                <div className="max-h-72 space-y-2 overflow-y-auto">
                  {pastChats.map((chat) => (
                    <button
                      key={chat.id}
                      type="button"
                      className="w-full rounded-lg border border-transparent bg-slate-50 px-3 py-3 text-left transition hover:border-slate-200 hover:bg-white"
                    >
                      ...
                      <p className="mt-1 truncate text-xs text-slate-600">{chat.preview}</p>
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-8 py-10 lg:px-10">
              <div className="mx-auto w-full max-w-6xl">
                <div className="mx-auto mb-5 flex h-12 w-12 items-center justify-center rounded-2xl border border-emerald-200 bg-emerald-50 text-emerald-600">
                  <Sparkles className="h-5 w-5" />
                </div>
                <h2 className="text-center font-display text-4xl font-semibold text-slate-900">
                  ...
                  {promptCards.map((item) => {
                    ...
                    return (
                      <button
                        key={item.id}
                        type="button"
                        className="cursor-pointer rounded-xl border border-slate-200 bg-slate-50 p-4 text-left transition hover:border-emerald-200 hover:bg-emerald-50/40"
                      >
                        ...
                        <p className="mt-3 text-xs font-semibold text-emerald-700">Try now</p>
                      </button>
                    )
                  })}
                </div>
              ...
              <div className="mx-auto w-full max-w-6xl">
                ...
                <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                  <textarea
                    ...
                    className="w-full resize-none bg-transparent text-sm text-slate-900 placeholder:text-slate-500 outline-none"
                  />
